...
const fetchDataStatusDate = async () => {
  try {
    const response = await fetch('https://sealling.iot4environment.com/admin/data_submersible.php');
    const data = await response.json();
    if (data && Array.isArray(data.result) && data.result.length > 0) {
      ...
      if (email && typeof email === 'string' && email.trim() !== '') {
        if (dangerousStatus.includes(status) && (lastStatusData[i] !== status || lastDateData[i] !== date)) {
          const mailOptions = {
            ...
            to: email,
            subject: `Peringatan sensor pressure dengan ${status} pada ${date}`,
...
const fetchDataAndSendNotificationsPressure = async () => {
  let lastStatusData = [];
  let lastDateData = [];
...
fetchDataAndSendNotificationsPressure();

module.exports = { fetchDataAndSendNotificationsPressure };
